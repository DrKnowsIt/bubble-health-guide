...
import { useAIFreeMode } from './useAIFreeMode';
import { useSessionPersistence } from './useSessionPersistence';
import { supabase } from '@/integrations/supabase/client';

export const useAIFreeModeEnhanced = (patientId?: string, selectedAnatomy?: string[]) => {
  const [healthTopics, setHealthTopics] = useState<any[]>([]);

  // Use the base AI Free Mode hook
  const aiFreeHook = useAIFreeMode(patientId, selectedAnatomy);
  
  // Session persistence integration
  const sessionId = `enhanced_${patientId || 'default'}_aifreechat`;
  const { saveSessionData, loadSessionData } = useSessionPersistence(sessionId);

  // Load persisted health topics on mount
  useEffect(() => {
    const savedData = loadSessionData();
    if (savedData?.healthTopics && savedData.healthTopics.length > 0) {
      console.log('Restoring health topics from localStorage:', savedData.healthTopics.length);
      setHealthTopics(savedData.healthTopics);
    }
  }, [loadSessionData]);

  // Save health topics to localStorage whenever they change
  useEffect(() => {
    if (healthTopics.length > 0) {
      saveSessionData({ healthTopics });
    }
  }, [healthTopics, saveSessionData]);
  ...
  const saveTopicsAsConversationDiagnoses = useCallback(async (topics: any[], currentPatientId?: string) => {
    ...
    try {
      const diagnosesToSave = topics.map(topic => ({
        ...
        reasoning: topic.reasoning || 'Generated from AI Free Mode session',
  ...
  const generateTopicsForConversation = useCallback(async (conversationPath: any[]) => {
    ...
    try {
      ...
      console.log('Generating real-time topics for AI Free Mode conversation', {
...
This summary was generated by DrKnowsIt AI to help prepare you for your medical consultation. Always consult with qualified healthcare professionals for medical advice.`;
  ...
  const completeSessionWithTopics = useCallback(async (path: any[], topics: any[]) => {
    if (!aiFreeHook.currentSession) return;
    ...
    try {
      ...
      await supabase
        ...
        .eq('id', aiFreeHook.currentSession.id);
  ...
  }, [aiFreeHook.currentSession, selectedAnatomy, saveTopicsAsConversationDiagnoses, patientId, generateSummary]);

  // Watch conversation path changes and generate topics
  useEffect(() => {
    if (aiFreeHook.conversationPath.length >= 2) {
      generateTopicsForConversation(aiFreeHook.conversationPath);
    }
  }, [aiFreeHook.conversationPath, generateTopicsForConversation]);

  // Re-trigger topic generation if conversation exists but no topics
  useEffect(() => {
    if (aiFreeHook.conversationPath.length >= 2 && healthTopics.length === 0) {
      console.log('Re-triggering topic generation for restored conversation');
      generateTopicsForConversation(aiFreeHook.conversationPath);
    }
  }, [aiFreeHook.conversationPath, healthTopics.length, generateTopicsForConversation]);
  ...
  const completeCurrentSession = useCallback(async () => {
    ...
    try {
      ...
      if (aiFreeHook.conversationPath.length < 10) {
        console.log(`Need at least 10 questions (currently ${aiFreeHook.conversationPath.length}). Continuing conversation...`);
        return; // Don't complete yet, need more questions
      }

      // After 10 questions, check if AI is confident about completion
      if (aiFreeHook.conversationPath.length >= 10 && aiFreeHook.conversationPath.length < 15) {
        const shouldComplete = await aiFreeHook.checkIfReadyToComplete?.(aiFreeHook.conversationPath);
        if (!shouldComplete) {
