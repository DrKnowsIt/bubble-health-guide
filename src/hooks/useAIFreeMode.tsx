...
export const useAIFreeMode = (patientId?: string, selectedAnatomy?: string[]) => {
  ...
  // Clear any existing session data on mount for fresh start
  useEffect(() => {
    if (user?.id) {
      clearSessionData();
      console.log('AI Free Mode: Cleared session data for fresh start');
    }
  }, [user?.id, clearSessionData]);
...
This summary was generated by DrKnowsIt AI to help prepare you for your medical consultation. Always consult with qualified healthcare professionals for medical advice.`;
  ...
  const generateClientFallbackQuestion = useCallback(async (
    ...
    // Find first question not already covered - be more strict about matching
    for (const question of smartGenericQuestions) {
      const alreadyAsked = askedQuestions.some(asked => {
        // Check for exact keyword matches and similar phrasing
        return question.keywords.some(keyword => asked.includes(keyword)) ||
               asked.includes(question.question.toLowerCase().substring(0, 15));
      });
  ...
  const startNewSession = useCallback(async (forceNew = false) => {
    ...
    // Skip session recovery in AI Free Mode - always start fresh
    if (!forceNew && !currentSession) {
      console.log('AI Free Mode: Skipping session recovery, always starting fresh');
    }
...
options: [
  ...
  const getResponseOptions = useCallback(() => {
    // If we have dynamic questions active (even for first question after anatomy selection), use those options
    if (useDynamicQuestions && dynamicQuestion && dynamicQuestion.options.length > 0) {
    ...
    // For the first question when NO anatomy selected and no dynamic question yet
    if (conversationPath.length === 0 && !dynamicQuestion) {
      return [
    ...
    const options = Object.entries(currentQuestion.response_leads_to)
      .filter(([value]) => value !== 'none_above' && value !== 'check_results');
