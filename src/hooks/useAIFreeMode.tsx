...
This summary was generated by DrKnowsIt AI to help prepare you for your medical consultation. Always consult with qualified healthcare professionals for medical advice.`;
  ...
  const generateClientFallbackQuestion = useCallback((
    ...
    const askedQuestions = path.map(p => p.question?.question_text || '').filter(Boolean);
    ...
    if (anatomy && anatomy.length > 0) {
      ...
      if (questionNumber <= 5) {
        // Early questions focus on main symptoms - check for duplicates
        const headQuestions = [
          {
            question: "When did these head-related symptoms first begin?",
            options: ["Within the last few days", "Within the last week", "Several weeks ago", "Months ago", "It's been ongoing for a long time", "I have other symptoms as well"]
          },
          {
            question: "How often do you experience these head symptoms?",
            options: ["Daily", "Several times a week", "Once a week", "Occasionally", "Only under certain conditions", "I have other symptoms as well"]
          },
          {
            question: "What time of day are your head symptoms most noticeable?",
            options: ["Morning", "Afternoon", "Evening", "Night", "No specific pattern", "I have other symptoms as well"]
          }
        ];
        ...
        const chestQuestions = [
          {
            ...
            options: ["Mild and occasional", "Moderate but manageable", "Severe and concerning", "Variable throughout the day", "Getting progressively worse", "I have other symptoms as well"]
          },
          {
            question: "Do your chest symptoms occur with physical activity?",
            options: ["Yes, mainly during activity", "Sometimes during activity", "Mainly at rest", "Both during activity and rest", "No clear pattern", "I have other symptoms as well"]
          }
        ];

        if (primaryArea.includes('head')) {
          const availableQuestions = headQuestions.filter(q => 
            !isSimilarQuestion(q.question, askedQuestions)
          );
          return availableQuestions[0] || headQuestions[questionNumber % headQuestions.length];
        } else if (primaryArea.includes('chest')) {
          const availableQuestions = chestQuestions.filter(q => 
            !isSimilarQuestion(q.question, askedQuestions)
          );
          return availableQuestions[0] || chestQuestions[questionNumber % chestQuestions.length];
        }
      } else {
...
options: [
    ...
    // Generic fallback questions with duplication prevention
    const genericQuestions = [
      {
        question: "Are there any specific triggers that make your symptoms worse?",
        options: ["Physical activity or movement", "Certain foods or drinks", "Stress or emotional situations", "Weather changes", "No clear triggers I've noticed", "I have other symptoms as well"]
      },
      {
        question: "Have you tried anything to help with your symptoms?",
        options: ["Over-the-counter medications", "Rest and lifestyle changes", "Home remedies", "Previously prescribed treatments", "Nothing has helped so far", "I have other symptoms as well"]
      },
      {
        question: "How are these symptoms affecting your sleep?",
        options: ["Not affecting sleep", "Difficulty falling asleep", "Waking up during the night", "Waking up tired", "Severe sleep disruption", "I have other symptoms as well"]
      },
      {
        question: "What activities are most difficult due to your symptoms?",
        options: ["Work tasks", "Physical exercise", "Social activities", "Daily household tasks", "No significant impact", "I have other symptoms as well"]
      },
      {
        question: "Have you noticed any patterns in when symptoms occur?",
        options: ["After eating", "During stress", "With weather changes", "At specific times of day", "No clear pattern", "I have other symptoms as well"]
      }
    ];
    
    // Filter out similar questions
    const availableQuestions = genericQuestions.filter(q => 
      !isSimilarQuestion(q.question, askedQuestions)
    );
    
    return availableQuestions.length > 0 
      ? availableQuestions[0] 
      : genericQuestions[questionNumber % genericQuestions.length];
  }, []);
