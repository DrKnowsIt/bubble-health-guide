...
export const ChatDashboard = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'ai',
      content: "Welcome back! I'm DrBubble, ready to help with your health questions. What's on your mind today?",
      timestamp: new Date()
    }
  ]);
  ...
  const exportConversation = () => {
    const conversationText = messages
      .map(msg => `${msg.type === 'user' ? 'You' : 'DrBubble'}: ${msg.content}`)
      .join('\n\n');
    ...
    a.href = url;
    a.download = `drbubble-conversation-${new Date().toLocaleDateString()}.txt`;
    a.click();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Chat with DrBubble</h1>
